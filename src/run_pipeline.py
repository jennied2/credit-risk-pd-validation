...
def run_pd():
    ...
    plt.barh(iv_sorted.index, iv_sorted["IV"], color=C_CHAMP)
    ...
    g2 = gimp.sort_values()
    plt.barh(g2.index, g2.values, color=C_CHAL)
    ...
    plt.barh(order.index[::-1], order.values[::-1], color=colors[::-1])
    ...
    for ax, feat in zip(axes, ["dti", "fico"]):
        ...
        ax.set_xlabel(feat); ax.set_ylabel("partial dependence (PD)")
